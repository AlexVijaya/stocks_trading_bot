...
def check_if_mirror_level_coincides_with_last_low(last_several_days_lows_slice_Series,
    ...
    if row_number_in_lows == last_several_days_lows_slice_Series.size - 1:
        ...
        print ( f"low_is_not_the_last_in_ohlcv_table with daily low = {daily_low}" )
        return False
...
def find_mirror_levels_in_database(database_where_stock_ohlcv_data_for_daily_df_is_contained,
    ...
    engine_for_stocks_ohlcv_db, connection_to_stocks_ohlcv =\
        connect_to_postres_db_without_first_deleting_it(database_where_stock_ohlcv_data_for_daily_df_is_contained)
    ###############################################33
    ###############################################
    ...
    engine_mirror_level_for_stocks_db , connection_to_mirror_level_for_stocks = \
        connect_to_postres_db_without_first_deleting_it ( db_where_mirror_levels_will_be_contained )
    ...
    try:
        drop_table ( table_where_mirror_levels_equal_to_last_high_or_low_will_be_contained ,
            ...
            for row_number_in_highs in range ( last_several_days_highs_slice_Series.size ):
                for row_number_in_lows in range ( last_several_days_lows_slice_Series.size ):
                    ...
                    if daily_high == daily_low:
                        ...
                        if not (last_low_equal_to_mirror_level_bool or last_high_equal_to_mirror_level_bool):
                            print("last low or high is not equal to mirror_level")
                            continue
                        # else:
                        #     mirror_level_df.loc[0 , 'last_low_or_high_is_equal_to_mirror_level'] = True
                        #     print ( f"last low at {daily_low} is equal to mirror level " )
                        ...
                        #     print ( f"last high at {daily_high} is equal to mirror level " )
                        #
                        #
                        ...
                        if timestamp_for_high!=timestamp_for_low:
                            ...
                            mirror_level_df.to_sql ( table_where_mirror_levels_equal_to_last_high_or_low_will_be_contained ,
                                                     engine_mirror_level_for_stocks_db ,
...
if __name__=="__main__":
    ...
    find_mirror_levels_in_database(database_where_stock_ohlcv_data_for_daily_df_is_contained,
                                   db_where_mirror_levels_will_be_contained,
                                   table_where_mirror_levels_equal_to_last_high_or_low_will_be_contained)

...
def add_8_and_9_only_column_to_df(data_df):
    # data_df["only_8_and_9s_seq_buy"]=np.NaN
    # data_df["only_8_and_9s_seq_sell"] = np.NaN

    data_df_slice=data_df.loc[(data_df["seq_buy"]>=8)|(data_df["seq_sell"] >= 8)]
    # data_df["seq_buy"]=data_df["seq_buy"].values.astype("int32").astype("str")
    # data_df["seq_sell"] = data_df["seq_sell"].values.astype ( "int32" ).astype("str")
    print(data_df.dtypes)
    data_df_slice_copy=data_df_slice.rename(columns={"seq_buy":"only_8_and_9s_seq_buy","seq_sell":"only_8_and_9s_seq_sell"})
    #data_df_slice_seq_sell = data_df.loc[data_df["seq_sell"] >= 8]
    print ( "data_df_slice_copy" )
    print ( data_df_slice_copy.head ( 5 ).to_string () )
    data_df_slice_copy=data_df_slice_copy[["only_8_and_9s_seq_buy","only_8_and_9s_seq_sell"]]
    data_df=pd.concat([data_df, data_df_slice_copy], axis=1)
    data_df["only_8_and_9s_seq_buy"].replace ( 0 , np.nan , inplace = True )
    data_df["only_8_and_9s_seq_sell"].replace ( 0 , np.nan , inplace = True )
    data_df['only_8_and_9s_seq_buy'] = data_df['only_8_and_9s_seq_buy'].astype('Int32').astype('str')
    data_df['only_8_and_9s_seq_sell'] = data_df['only_8_and_9s_seq_sell'].astype('Int32').astype('str')
    # for row_number in range(0,len(data_df_slice)):
    #     print("data_df[[row_number]]")
    #     timestamp_of_seq=data_df_slice.iat[row_number,"Timestamp"]
    #     # data_df["only_8_and_9s_seq_sell"]=
    #     # print ( data_df.iloc[[row_number]] )
    print ( "data_df" )
    print ( data_df.head(300).to_string () )
...
def plot_ohlcv_chart_with_levels_formed_by_ath (name_of_folder_where_plots_will_be,
    ...
    for row_with_level_formed_by_ath in range ( 0 , len ( table_of_levels_formed_by_ath_df ) ):
        ...
        try:
            ...
            # print("data_df_slice_seq_buy")
            # print ( data_df_slice_seq_buy.to_string() )
            # print ( "data_df_slice_seq_sell" )
            # print ( data_df_slice_seq_sell.to_string () )
            print ( "data_df" )
            ...
            try:
                ...
                try:
                    ...
                    # fig.add_scatter (
                    #     x = data_df_slice_seq_sell_several_last_days_are_left["open_time"] ,
                    #     y = data_df_slice_seq_sell_several_last_days_are_left["high"] ,
                    #     mode = "markers+text" ,
                    #     marker = dict ( color = "rgba(255, 0, 0, 0)" , size = 15 ) ,
                    #     opacity = 1 ,
                    #     text = data_df_slice_seq_sell_several_last_days_are_left["seq_sell"] ,
                    #     textposition = 'top center' ,
                    #     textfont = dict ( color = "#FF0000" , size = 20 , ) ,
                    # 
                    #     name = "td_count_for_highs" , row = 2 , col = 1 )

                except Exception as e:
                    print ( "error" , e )
                    traceback.print_exc ()

                #plot horizontal line at ath

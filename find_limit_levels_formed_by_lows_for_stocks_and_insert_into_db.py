...
def count_how_many_lows_lower_than_level_formed_by_lows_asset_had(engine_for_ohlcv_data_for_stocks,
                                      table_name_with_ohlcv_data, level_formed_by_low):
    table_with_ohlcv_data_df = \
        pd.read_sql_query ( f'''select * from "{table_name_with_ohlcv_data}"''' ,
                            engine_for_ohlcv_data_for_stocks )
    print("table_with_ohlcv_data_df")
    print ( table_with_ohlcv_data_df )

    how_many_lows_lower_than_level_formed_by_lows_asset_had =\
        np.count_nonzero ( table_with_ohlcv_data_df["low"] < level_formed_by_low )
...
def find_limit_levels_formed_by_lows_for_stocks(so_many_number_of_touches_of_level_by_highs,
    ...
    # open_connection to database where levels formed by lows will be stored
    ...
    for number_of_gaps_before_low in range(0,4):
        try:
            drop_table ( f"{table_where_levels_formed_by_lows_will_be}_with_{number_of_gaps_before_low}_number_of_gaps" ,
                         engine_for_levels_formed_by_highs_and_lows_in_stocks )
            print ( "\ntable dropped\n" )
            # time.sleep ( 1000 )
        except Exception as e:
    ...
    # drop table with levels formed by lows and which had that many false breakouts
    for number_of_lows_to_check_with in [0 , 1 , 2 , 3 , 4 , 5]:
        table_where_levels_which_had_lows_lower_than_the_level_will_be_stored = \
            f"levels_with_{number_of_lows_to_check_with}_number_of_lows_lower_than_level_formed_by_lows"
        drop_table ( table_where_levels_which_had_lows_lower_than_the_level_will_be_stored ,
                     engine_for_levels_formed_by_highs_and_lows_in_stocks )
    ...
    levels_formed_by_lows_df = pd.DataFrame ( columns = column_list )
    ...
    for table_in_db in list_of_tables:
        ...
        try:
            ...
            last_several_days_slice_of_df_for_average_volume_calculation_and_removal_of_duplicates = data_df.tail ( this_many_last_days_for_data_df_slice )
            ...
            for row_of_low_levels in range(len(data_df_slice_plus_number_of_equal_lows_without_NaNs)):
                ...
                try:
                    ...
                    if not (next_index_of_low>last_index_of_data_df or\
                        ...
                        #################################
                        #################################
                        if current_low not in dict_of_lows_with_timestamps:
                            dict_of_lows_with_timestamps[current_low]=[timestamp_for_low_level]
                        else:
                            dict_of_lows_with_timestamps[current_low].append(timestamp_for_low_level)
            ...
            print ( "dict_of_lows_with_timestamps\n" ,
                    dict_of_lows_with_timestamps )
            ...
            for level in dict_of_lows_with_timestamps.keys ():
                counter = 0
                ...
                # for legit_or_not_legit_string in dict_of_lows_with_timestamps[level]:
                #
                #     counter = counter + 1
                #     print ( f"{level} has no non legit"
                #             f" lows in dict {dict_of_lows_with_timestamps[level]}" )
                #     # del dict_of_lows_with_timestamps[level]
                #     print ( "key deleted successfully " )
                #
                #     print ( "counter_number_of_legit_level_occurrences" ,
                #             counter )
                #     # number_of_legit_level_occurencies=len(dict_of_lows_with_timestamps[level])
                ...
                #         dict_of_lows_legit_or_not_legit_with_only_legit_level )
                if len ( dict_of_lows_with_timestamps ) > 0:
                    # for level in dict_of_lows_with_timestamps.keys ():
                    ...
                    levels_formed_by_lows_with_certain_number_of_lows_lower_than_level = pd.DataFrame (
                        columns = column_list )

                    how_many_lows_lower_than_level_formed_by_lows_asset_had=\
                        count_how_many_lows_lower_than_level_formed_by_lows_asset_had(engine_for_stocks_ohlcv,
                                                                                  stock_ticker,
                                                                                  level)


                    for timestamp in \
                    dict_of_lows_with_timestamps[level]:
                    ...
                    if how_many_lows_lower_than_level_formed_by_lows_asset_had in range ( 0 , 6 ):
                        last_row_of_levels_formed_by_lows_df = levels_formed_by_lows_df.iloc[-1:]
                        table_where_levels_formed_by_lows_will_be_for_false_breakout_calculation = \
                            f"levels_with_{how_many_lows_lower_than_level_formed_by_lows_asset_had}_number_of_lows_lower_than_level_formed_by_lows"
                        last_row_of_levels_formed_by_lows_df.to_sql ( table_where_levels_formed_by_lows_will_be_for_false_breakout_calculation ,
                                                                      connection_to_levels_formed_by_highs_and_lows_in_stocks ,
                                                                      if_exists = 'append' ,
                                                                      index = False )
                    ...
                    if min_number_of_gaps_before_any_low_for_stock_not_to_be_discarded in list_of_numbers_of_gaps_before_low:
                        levels_formed_by_lows_df_with_1_gap.to_sql (
                            f"{table_where_levels_formed_by_lows_will_be}_with_{min_number_of_gaps_before_any_low_for_stock_not_to_be_discarded}_number_of_gaps" ,
                            connection_to_levels_formed_by_highs_and_lows_in_stocks ,
                            if_exists = 'append' ,
                            index = False )

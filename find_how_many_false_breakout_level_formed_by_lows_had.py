...
def count_how_many_lows_lower_than_level_formed_by_lows_asset_had(engine_for_ohlcv_data_for_stocks,
                                      table_with_ohlcv_table, level_formed_by_low):
    table_with_ohlcv_data_df = \
        pd.read_sql_query ( f'''select * from "{table_with_ohlcv_table}"''' ,
                            engine_for_ohlcv_data_for_stocks )
    print("table_with_ohlcv_data_df")
...
def insert_into_db_levels_which_had_so_many_lows_lower_than_level_formed_by_lows(
    ...
    for row_of_lows in range ( 0 , len ( table_with_all_low_levels_df ) ):
        ...
        try:
            ...
            print ("level_formed_by_low=" ,level_formed_by_low )
            ...
            that_many_lows_lower_than_level_formed_by_lows_asset_had=\
                count_how_many_lows_lower_than_level_formed_by_lows_asset_had ( engine_for_ohlcv_data_for_stocks ,
                                                                            table_with_ohlcv_table ,
                                                                            level_formed_by_low )

            for number_of_lows_to_check_with in [0,1,2,3,4,5]:
                if that_many_lows_lower_than_level_formed_by_lows_asset_had==number_of_lows_to_check_with:
                    table_where_levels_which_had_lows_lower_than_the_level_will_be_stored =\
                        f"levels_with_{number_of_lows_to_check_with}_number_of_lows_lower_than_level_formed_by_lows"
                    print (f"stock = {stock_ticker} on exchange = {exchange} had "
                           f"{that_many_lows_lower_than_level_formed_by_lows_asset_had} lows lower than level = {level_formed_by_low}")
                    one_row_df.to_sql ( table_where_levels_which_had_lows_lower_than_the_level_will_be_stored ,
                                                      connection_to_table_where_level_formed_by_lows_are_stored ,
...
if __name__=="__main__":
    ...
    # table_where_levels_which_had_lows_lower_than_the_level_will_be_stored="levels_which_had_lows_lower_than_the_limit_level_formed_by_lows"



    insert_into_db_levels_which_had_so_many_lows_lower_than_level_formed_by_lows(db_where_levels_formed_by_lows_are_stored,
                                                    table_where_levels_formed_by_lows_are_stored,
                                                    db_where_ohlcv_data_for_stocks_is_stored)
    end_time = time.time ()

...
def find_levels_formed_by_non_whipsawed_gaps(percentage_between_gap_level_and_closing_price,
    ...
    for stock_name in list_of_tables_in_ohlcv_db:
        ...
        for row_number,row_in_numpy_array in enumerate(table_with_ohlcv_data_df_slice_numpy):
            # find gaps down
            try:
                prev_low=table_with_ohlcv_data_df_slice_numpy[row_number-1][2]
                #print("row_in_numpy_array=",row_in_numpy_array)
                current_high=row_in_numpy_array[1]
                if (prev_low-current_high)>0:
                    print (f"{stock_name} had a gap down")
                    max_high_after_gap_down=np.NaN
                    min_low_before_gap_down=np.NaN

                    try:
                        numpy_array_of_lows_before_gap_down=\
                            table_with_ohlcv_data_df_slice_numpy[0:row_number-1,2]
                        print ( "numpy_array_of_lows_before_gap_down=" , numpy_array_of_lows_before_gap_down )

                        #deal with situation if level formed by low is first bar in all data
                        if len(numpy_array_of_lows_before_gap_down)==0:
                            min_low_before_gap_down=prev_low
                        else:
                            min_low_before_gap_down=np.amin(numpy_array_of_lows_before_gap_down)
                            print("min_low_before_gap_down")
                            print ( min_low_before_gap_down )
                    except:
                        traceback.print_exc()
                    try:
                        numpy_array_of_highs_after_gap_down =\
                            table_with_ohlcv_data_df_slice_numpy[row_number + 1:len( table_with_ohlcv_data_df_slice_numpy )-1 ,1]
                        # print ( "numpy_array_of_highs_after_gap_down=" , numpy_array_of_highs_after_gap_down )

                        #if gap took place today
                        if len(numpy_array_of_highs_after_gap_down)==0:
                            max_high_after_gap_down=current_high
                        else:
                            max_high_after_gap_down = np.amax ( numpy_array_of_highs_after_gap_down )
                    except:
                        traceback.print_exc()
                    ...
                    if ((current_high>=max_high_after_gap_down) and (prev_low<=min_low_before_gap_down)):
                        timestamp_for_prev_low=table_with_ohlcv_data_df_slice_numpy[row_number-1][0]
                        timestamp_for_current_high = table_with_ohlcv_data_df_slice_numpy[row_number][0]
                        prev_low_open_date_with_time,prev_low_open_date_without_time=\
                            get_date_with_and_without_time_from_timestamp ( timestamp_for_prev_low )
                        current_high_open_time_with_time, current_high_open_date_without_time=\
                            get_date_with_and_without_time_from_timestamp ( timestamp_for_current_high )
                        print(f"{stock_name} had a gap down with low={prev_low} on {prev_low_open_date_without_time} "
                              f"and high={current_high} on {current_high_open_date_without_time}."
                        ...
                        level_formed_by_gaps_which_were_not_whipsawed_df.loc[
                            0 , 'exchange'] = exchange
                        level_formed_by_gaps_which_were_not_whipsawed_df.loc[
                            0 , "gap_level_lower_border"] = current_high
                        level_formed_by_gaps_which_were_not_whipsawed_df.loc[
                            0 , "gap_level_higher_border"] = prev_low
                        level_formed_by_gaps_which_were_not_whipsawed_df.loc[
                            0 , "short_name"] = short_name
                        level_formed_by_gaps_which_were_not_whipsawed_df.loc[
                            0 , "timestamp_lower_border"] = current_high_open_date_without_time
                        level_formed_by_gaps_which_were_not_whipsawed_df.loc[
                            0 , "timestamp_higher_border"] = prev_low_open_date_without_time
                        level_formed_by_gaps_which_were_not_whipsawed_df.loc[
                    ...
                    elif ((current_high>=max_high_after_gap_down) and (prev_low>min_low_before_gap_down)):
                        if current_high<min_low_before_gap_down:
                            timestamp_for_prev_low = table_with_ohlcv_data_df_slice_numpy[row_number - 1][0]
                            timestamp_for_current_high = table_with_ohlcv_data_df_slice_numpy[row_number][0]
                            prev_low_open_date_with_time , prev_low_open_date_without_time = \
                                get_date_with_and_without_time_from_timestamp ( timestamp_for_prev_low )
                            current_high_open_time_with_time , current_high_open_date_without_time = \
                                get_date_with_and_without_time_from_timestamp ( timestamp_for_current_high )
                            print (
                                f"{stock_name} had a gap down with low={prev_low} and high={current_high}."
                                f"Level formed by the first high after gap is B+ quality but "
                                f"level formed by low is B because it was whipsawed" )
                    ...
                    # Low level is good . High level is bad
                    elif ((prev_low<=min_low_before_gap_down) and (current_high<max_high_after_gap_down)):
                        if prev_low>max_high_after_gap_down:
                            timestamp_for_prev_low = table_with_ohlcv_data_df_slice_numpy[row_number - 1][0]
                            timestamp_for_current_high = table_with_ohlcv_data_df_slice_numpy[row_number][0]
                            prev_low_open_date_with_time , prev_low_open_date_without_time = \
                                get_date_with_and_without_time_from_timestamp ( timestamp_for_prev_low )
                            ...
                            print (
                                f"{stock_name} had a gap down with low={prev_low} and high={current_high}."
                                f"Level formed by the low before gap is B+ quality.  "
...
if __name__=="__main__":
    ...
    table_where_levels_formed_by_gap_level_will_be = "levels_formed_by_gap_down"
